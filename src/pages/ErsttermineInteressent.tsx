...
import { ClipboardList, Plus, Calendar, Filter, Search, User } from "lucide-react";
...
const ErsttermineInteressent = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const [termine, setTermine] = useState<ErstterminInteressent[]>([]);
  ...
  const fetchTermine = async () => {
    ...
    if (data && data.length > 0) {
      const customerIds = [...new Set((data as ErstterminInteressent[]).map((t) => t.customer_id).filter(Boolean))];
      ...
      const enriched = (data as ErstterminInteressent[]).map((t) => ({
        ...t,
  ...
  const getEntscheidungsBadge = (entscheidungsstatus: string | null) => {
    if (!entscheidungsstatus) return null;
    switch (entscheidungsstatus) {
      case "offen":
        return <Badge variant="outline">Offen</Badge>;
      case "interessiert":
        return <Badge className="bg-yellow-500 text-white">Interessiert</Badge>;
      case "zugesagt":
        return <Badge className="bg-green-500 text-white">Zugesagt</Badge>;
      case "abgesagt":
        return <Badge className="bg-red-500 text-white">Abgesagt</Badge>;
      default:
        return <Badge variant="outline">{entscheidungsstatus}</Badge>;
    }
  };
  ...
  const filteredTermine = termine.filter((t) => {
    const matchesSearch =
      ...
      (t.projektart || "").toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageHeader title="Ersttermine - Interessenten" />
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              Ersttermine - Interessenten
            </h1>
            <p className="text-muted-foreground">Ersttermine mit Interessenten verwalten</p>
          </div>
          <Button onClick={() => navigate("/ersttermine-interessent/neu")} className="gap-2">
          ...
          <CardContent className="pt-4">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Suche nach Kunde, Nummer, Projektart..."
                  value={searchQuery}
        ...
        {filteredTermine.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              {!searchQuery && statusFilter === "alle" && (
                <Button onClick={() => navigate("/ersttermine-interessent/neu")} variant="outline">
          ...
          <div className="space-y-4">
            {filteredTermine.map((termin) => (
              <Card
                ...
                onClick={() => navigate(`/ersttermine-interessent/${termin.id}`)}
              >
                <CardContent className="pt-4">
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          <h3 className="font-semibold text-lg">
                            {termin.customer_name || "Kein Kunde"}
                          </h3>
                          {termin.nummer && (
                        ...
                        <div className="flex gap-2">
                          {getEntscheidungsBadge(termin.entscheidungsstatus)}
...
export default ErsttermineInteressent;

...
const BUCKET = "besprechungsprotokoll-photos";
const TABLE = "besprechungsprotokoll_photos";

export const BesprechungsprotokollPhotos = ({ protokollId }: Props) => {
  const { toast } = useToast();
  const [photos, setPhotos] = useState<ProtoPhoto[]>([]);
  ...
  const fetchPhotos = async () => {
    ...
    const { data, error } = await (supabase.from(TABLE as never) as any)
      .select("*")
      .eq("besprechungsprotokoll_id", protokollId)
      .order("created_at", { ascending: false });
    if (!error && data) setPhotos(data as ProtoPhoto[]);
    setLoading(false);
  };
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      if (!file.type.startsWith("image/")) {
        toast({ variant: "destructive", title: "Ungültiger Dateityp", description: `${file.name} ist kein Bild` });
        continue;
      }
      if (file.size > 10 * 1024 * 1024) {
        toast({ variant: "destructive", title: "Datei zu groß", description: `${file.name} ist größer als 10MB` });
        continue;
      }

      const fileName = `${protokollId}/${Date.now()}_${file.name}`;
      ...
      const { error: dbError } = await (supabase.from(TABLE as never) as any).insert({
        besprechungsprotokoll_id: protokollId,
        user_id: user.id,
        file_path: fileName,
        file_name: file.name,
      });
      ...
      if (dbError) {
        ...
        toast({ variant: "destructive", title: "Fehler", description: "Foto konnte nicht gespeichert werden" });
        continue;
      }
      uploadedCount++;
    }
  ...
  const handleDelete = async (photo: ProtoPhoto) => {
    await supabase.storage.from(BUCKET).remove([photo.file_path]);
    const { error } = await (supabase.from(TABLE as never) as any).delete().eq("id", photo.id);
    if (error) {
  ...
  const handleUpdateBeschreibung = async (photoId: string, beschreibung: string) => {
    await (supabase.from(TABLE as never) as any).update({ beschreibung }).eq("id", photoId);
    setPhotos((prev) => prev.map((p) => (p.id === photoId ? { ...p, beschreibung } : p)));
  };
    ...
    <>
      ...
      <Dialog open={!!selectedPhoto} onOpenChange={() => setSelectedPhoto(null)}>
        <DialogContent className="max-w-[95vw] max-h-[95vh] p-0 overflow-hidden">
          ...
          {selectedPhoto && (
            <img src={selectedPhoto} alt="Vollbild" className="w-full h-full object-contain max-h-[90vh]" />
          )}
        </DialogContent>
      </Dialog>
    </>
